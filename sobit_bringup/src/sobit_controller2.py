...
def cul_motion(vel,ang,position):
			...
			#rad2deg			
			for i in range(1,31):
				position[i] = position[i] / 3.1415 * 180
			...
			motion[1] = '%04x' %(32768 + motion_deg[0] * 44)		#<L_wheel>
			motion[2] = '%04x' %(32768 - motion_deg[1] * 44)		#<R_wheel>
...
####callback2########################################################################################################
def callback2(position):
			global state_jointstate

			#print position
			state_jointstate = position
...
def callback(cmd_vel):
			...
			#関節position指示の取得
			sub = rospy.Subscriber('/joint_states', JointState, callback2)
			...
			#速度から距離とモーションタイムを計算
			motion = cul_motion(cmd_vel.linear.x,cmd_vel.angular.z,state_jointstate.position)
