...
def cul_motion(vel,ang,position):
			global L_motion_vel, R_motion_vel, L_motion_cm, R_motion_cm
			global D
			global TIME, CMD_flag
			motion = [0]*21
			motion_deg = [0]*21
			motion_in = 0
...
def get_enc():
			...
			#position_enc[1] = position_enc[1]/32*360
			#position_enc[2] = position_enc[2]/32*360
...
def callback1(jointstate):
			...
			global VEL
			global ANG
			pub = rospy.Publisher('sobit_enc', JointState , queue_size=10)		#publisherの定義	
...
def callback2(cmd_vel):		
			global VEL, ANG
			global TIME, CMD_TIME, CMD_first_flag ,CMD_flag
			print "\n\n[CALL_BACK2]"

			VEL = cmd_vel.linear.x
			ANG = cmd_vel.angular.z

			if CMD_first_flag == True:
				CMD_TIME = rospy.Time.now()
				CMD_first_flag = False

			now = rospy.Time.now()
			delta_T = now - CMD_TIME
